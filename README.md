...
## 2026.06.17.28 全面修复与回归检查

- 补回遗漏的 `languages.js`，修复多语言脚本无法加载；
- 修复 `<title>` 中嵌套 `<span>` 的无效HTML；
- 修复 `<option>` 中嵌套 `<span>` 的兼容性问题；
- 将24语言选择器恢复到标题旁，删除重复控件；
- 更新CSS/JS缓存版本号；
- 保留并复查护士管区禁止跨物理排的双重防护；
- 新增跨排运行时安全断言；
- 未主动修改现有排班策略、患者规则和界面布局。


## 2026.06.17.29 语言切换卡死修复

根因是运行时翻译的 `MutationObserver` 监听了文本变化，同时翻译函数即使没有找到译文、结果与原文完全相同，也会把同一段文字重新写入DOM。该写入再次触发观察器，形成高频无限循环，导致浏览器和整个系统CPU占用异常。
...
- 翻译期间临时断开观察器；
- 多个DOM变化合并到同一动画帧批量处理；
- 切回中文或关闭页面时主动停止观察器并取消待处理任务；
- 保留24语言、RTL布局和现有排班逻辑不变。
